...
규칙:
1. parties: 법적 주체만. 목적물(X토지, 어음)은 제외. 역할어(원고/피고/피고인/피해자/회사)도 당사자로 포함. 법원·검찰은 제외.
2. relations의 label은 반드시 이 통제 어휘 중 하나로: ${VOCAB.join(', ')}.
3. 방향은 라벨의 의미 기준: 매도=매도인→매수인, 임대=임대인→임차인, 금전대여=대여자→차용자, 담보류(근저당권 설정·보증·담보 제공 등)=제공자→권리자, 위임·도급·고용=위임자(도급인·고용주)→수탁자, 양도·채권양도=양도인→양수인, 기망·횡령 등 범죄=가해자→피해자, 상속=피상속인→상속인, 유증·증여=주는 자→받는 자.
4. 수동·수취 표현("설정받았다","매수하였다","위탁받았다")도 방향 규칙을 따라 정규화하라.
5. date는 지문의 날짜. 연도·월·일 형식("2020. 3. 1."). 없으면 null.
6. obj는 특정 목적물 명칭(X토지, A주택, 약속어음). 금전 액수는 label 괄호에 넣지 말고 생략.
7. 부정("지급하지 않았다")·미실현("인도하려고")은 관계에서 제외. 통지·판결·항소 등 절차는 events에만.
8. 하나의 법률행위 = 1개 관계. 중복 생성 금지.`;
}
